...
res =  fD.querySelect(	typeConn='source',
						querySource= 	"""	SELECT 
											...
											FROM  
											[SCR_RT]
											WHERE [date] >=  CONVERT(varchar(10), GETDATE()-7, 120) 
											GROUP BY msc, date 
											""" )
...
for mscNow in listMsc: 
	dfPrint = pd.pivot_table(dfUseKpi[dfUseKpi['msc'] == mscNow] , values='scr', index=['time'],  columns=['system','pool','msc'], aggfunc='max')
	...
	plt.savefig('xSCR-' + titleMscGraph + '.png')
	plt.clf()
	plt.cla()
	plt.close()