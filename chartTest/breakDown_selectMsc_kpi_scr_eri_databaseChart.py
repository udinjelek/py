...
listMscUse = ['MSSB9']
for mscNow in listMscUse: 
	for kpiUseNow in listKpiUse: 
		dfPrint = pd.pivot_table(dfUseKpi[dfUseKpi['msc'] == mscNow] , values=kpiUseNow, index=['time'],  columns=['system','pool','msc'], aggfunc='max')
		...
		plt.title(label=kpiUseNow + ' ' + titleMscGraph)
		...
		plt.savefig('BreakEri_SCR-'+kpiUseNow +'-'+ titleSysGraph + '-' +titleMscGraph+'.png')
		plt.clf()
		plt.cla()
		plt.close()
