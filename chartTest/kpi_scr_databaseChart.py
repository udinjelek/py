...
res =  fD.querySelect(typeConn='source',querySource="select XDATE, POOL, NE, SCR_MONITORING, [call attempt times (times)] from WEB_SCR_MSC_HW_HR where XDATE>= CONVERT(varchar(10), GETDATE()-7, 120)" ) 
for rx in res:
...
res =  fD.querySelect(typeConn='target',querySource="select time, pool, ne, [SCR (%)], [call_Attempt (times)] from  V_WEBCESOC_MSC_ER_SCR_HOURLY where time >= CONVERT(varchar(10), GETDATE()-7, 120)" ) 
for rx in res:
...
for poolNow in listPool: 
	...
	plt.savefig('SCR-' + titleSysGraph + '-' +titlePoolGraph+'.png')
	plt.clf()
	plt.cla()
	plt.close()
