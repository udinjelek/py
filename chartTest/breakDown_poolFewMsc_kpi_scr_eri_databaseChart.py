...
# res =  fD.querySelect(typeConn='target',querySource="select time, pool, ne, [SCR (%)], [call_Attempt (times)], [call_Success (times)]  from  V_WEBCESOC_MSC_ER_SCR_HOURLY where time >= CONVERT(varchar(10), GETDATE()-3, 120)" ) 
res =  fD.querySelect(typeConn='target',querySource="select time, pool, ne, [SCR (%)], [call_Attempt (times)], [call_Success (times)]  from  V_WEBCESOC_MSC_ER_SCR_HOURLY where time >= GETDATE()-1" ) 
for rx in res:
...
listMscUse = ['MSJ24','MSJ25','MSJ26',]
dfMscUse = pd.DataFrame(listMscUse, columns =['msc']) 
dfUseKpi = pd.merge(dfMscUse, dfUseKpi, on = 'msc')
dfPoolMsc = pd.merge(dfMscUse, dfDoMsc, on = 'msc')

exit
start3_time = time.time()

listPool = dfPoolMsc.pool.unique()
print(listPool)
...
for poolNow in listPool: 
	for kpiUseNow in listKpiUse: 
		...
		plt.savefig('BreakEri_SCR-'+kpiUseNow +'-'+ titleSysGraph + '-' +titlePoolGraph+'-SelectedMsc.png')
		plt.clf()
		plt.cla()
		plt.close()
